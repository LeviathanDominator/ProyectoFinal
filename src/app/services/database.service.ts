...
export class DatabaseService {
    ...
    private fireSql: FireSQL = new FireSQL(firebase.firestore());
    nameLength = 30; // Max length for name input.
    filters: Filter[]; // Filters for labels.
    labelsLength: number; // Number of labels in database.
    ...
    suggestLabel(userId: string, gameId: number, labelsData: any): Promise<void> {
        const labels = [];
    ...
    getAverageLabels(gameId: number): Promise<any> {
    ...
    getAverageLabelsData(game: Game): void {
    ...
    getList(id: string, userId: string): Observable<any> {
        return this.firestore.collection('/users').doc(userId).collection('/lists').doc(id).valueChanges();
    }

    // Sets a new filter based on the data received.
    ...
    newList(userId: string, name: string, gameId?: number): Promise<void> {
    ...
    addGameToList(list: List, game: Game): void {
    ...
    updateList(userId: string, list: List, games?: Game[]): Promise<void> {
        const gameIds = [];
        if (games) {
    ...
    dataToList(data: any): List {
        return new List(data.id, data.name, data.games);
    }

    // Turns user data into an User object.
    dataToUser(data: any): User {
    ...
    sendMessage(senderId: string, receiverId: string, title: string, messageString: string): Promise<void> {
        const date = new Date();
    ...
    deleteList(userId: string, list: List): void {
    ...
    deleteMessage(message: Message): Promise<void> {
        return this.firestore.collection('/users').doc(message.receiver).collection('/messages').doc(message.id).delete();
    }
    ...
    numUnreadMessages(messages: any): number {
        let numMessages = 0;
    ...
    dataToMessage(data: any): Message {
    ...
    currentTimeAndDate(raw: boolean): string {
    ...
    private exist(labels: Label[], i: number): boolean {
        for (const label of labels) {
    ...
    async toast(message: string, name?: string): Promise<void> {
        if (!name) {
            name = '';
        }
    ...
    async noConnectionAlert(): Promise<void> {
    ...
    async customAlert(header: string, message: string): Promise<void> {
        const alert = await this.alertController.create({
            header,
            message,
            buttons: [
